...
元组（tuple）是不可变序列，可以包含不同类型的元素。支持索引、切片等操作，但元素不可修改。
...
字典（dict）是键值对集合，支持元素添加、修改、删除、访问等操作。
...
print(student)  # 输出: {'name': 'Alice', 'age': 26, 'grade': 'A', 'major': 'Computer Science'}
...
print(values)  # 输出: dict_values(['Alice', 26, 'Computer Science'])
```

### 集合（set）操作

集合（set）是包含不同类型的元素的无序集合，支持元素添加、删除、交集、并集等操作。
...
通过掌握变量的定义和使用，以及Python中常见的数据类型和类型转换，你将能够更有效地处理和操作数据，编写功能强大且灵活的Python代码，为更高级的编程概念打下坚实基础。
